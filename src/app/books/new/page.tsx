...
export default function NewBookPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.title || !form.description || !form.price) {
      setError(t('new_book.book_title') + ', ' + t('new_book.description') + ' и цена са задължителни')
      return
    }
    setLoading(true)
    try {
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6">
          <h2 className="font-semibold text-stone-700 mb-4">📷 {t('new_book.scan_title').replace('📷 ', '')}</h2>
          <ImageUpload images={images} onChange={setImages} />
        </div>

        {/* Basic info */}
        <div className="bg-white rounded-2xl border border-stone-200 p-6 space-y-4">
          <h2 className="font-semibold text-stone-700">{t('new_book.basic_info')}</h2>
          <Input label={t('new_book.book_title')} id="title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Напр. Добротолюбие" required />
          <div>
            <label className="text-sm font-medium text-stone-700">{t('new_book.description')}</label>
            <textarea
          ...
          />
          <Select label={t('new_book.category')} id="categoryId" value={form.categoryId} onChange={(e) => setForm({ ...form, categoryId: e.target.value })} options={categoryOptions} />

          {/* Publisher with inline add */}
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-sm font-medium text-stone-700">{t('new_book.publisher')}</label>
              <button
                type="button"
        ...
        {/* Edition details */}
        <div className="bg-white rounded-2xl border border-stone-200 p-6 space-y-4">
          <h2 className="font-semibold text-stone-700">{t('new_book.edition')}</h2>
          <div className="grid grid-cols-2 gap-4">
            <Select label={t('new_book.year')} id="period" value={form.period} onChange={(e) => setForm({ ...form, period: e.target.value })} options={periodOptions} />
            <Input label={t('new_book.year')} id="year" type="number" value={form.year} onChange={(e) => setForm({ ...form, year: e.target.value })} placeholder="2023" min="100" max="2100" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <Input label={t('new_book.pages')} id="pages" type="number" value={form.pages} onChange={(e) => setForm({ ...form, pages: e.target.value })} placeholder="350" />
            <Input label={t('new_book.isbn')} id="isbn" value={form.isbn} onChange={(e) => setForm({ ...form, isbn: e.target.value })} placeholder="978-954-..." />
          </div>
        </div>

        {/* Price & availability */}
        <div className="bg-white rounded-2xl border border-stone-200 p-6 space-y-4">
          <h2 className="font-semibold text-stone-700">{t('new_book.pricing')}</h2>
          <div className="grid grid-cols-2 gap-4">
            <Input label={t('new_book.price')} id="price" type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="25.00" min="1" step="0.01" required />
            <Input label={t('new_book.original_price')} id="originalPrice" type="number" value={form.originalPrice} onChange={(e) => setForm({ ...form, originalPrice: e.target.value })} placeholder="35.00" min="0" step="0.01" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <Select label={t('new_book.condition')} id="condition" value={form.condition} onChange={(e) => setForm({ ...form, condition: e.target.value })} options={conditionOptions} />
            <Input label={t('new_book.stock')} id="stock" type="number" value={form.stock} onChange={(e) => setForm({ ...form, stock: e.target.value })} min="1" max="999" />
          </div>
        </div>
